# Projekt Techniczny (Design Initial): Integracja Portalu Nieruchomosci-online.pl
...
* **Audyt Kompletności**: Ekstrakcja liczby ofert deklarowanej przez portal (`expected_total`) i rejestracja wskaźnika kompletności w tabeli `run_audit`.
...
Portal Nieruchomosci-online.pl wykorzystuje autorski schemat pozycyjny parametrów rozdzielanych przecinkami. Na podstawie analizy endpointów portalu, generator zapytań URL musi realizować następującą strukturę:
...
> [!NOTE]
> **[Hipoteza/Domysł]**: Dokładna kolejność slotów pozycyjnych parametrów w `szukaj.html` bazuje na obserwacji formatu linków URL portalu Nieruchomosci-online.pl. Implementacja generatora URL w klasie `NieruchomosciOnlineProvider` musi posiadać wyizolowaną metodę `build_search_url(...)`, podlegającą dedykowanym testom jednostkowym.
...
Podobnie jak w `AdresowoProvider`, najwyższą jakość danych gwarantuje dwufazowy proces ekstrakcji:
...
Nieruchomosci-online.pl monitoruje nagłówki i częstotliwość zapytań. Provider musi wysyłać pełen zestaw realistycznych nagłówków przeglądarki:
```python
HEADERS = {
...
```
Dodatkowo wymagane jest stosowanie odstępów czasowych (`time.sleep(0.2 - 0.5)` sekundy) pomiędzy kolejnymi zapytaniami o szczegóły ofert.

---

## 4. Kontrakty Danych, Schemat `raw_payload` i Widoki SQLite (`db.py`)

### 4.1. Schemat `raw_payload` dla `nieruchomosci_online`
Do tabeli `bronze_listings` zapisywany jest surowy payload w formacie JSON o ustandaryzowanej strukturze kluczy:
...
```json
{
  ...
  "description_text": "Jasne, przestronne mieszkanie trzypokojowe...",
  "location": {
    "city": "Warszawa",
    "district": "Ursynów",
    "street": "ul. Dereniowa",
...
Istniejący widok `silver_listings` w [`db.py`](file:///Users/pawel/git/gen-ai-orchestrator/wyszukiwarka-nieruchomosci/src/db.py) został zaprojektowany w sposób elastyczny i odczytuje klucze:
* `title`: `COALESCE(json_extract(b.raw_payload, '$.title'), json_extract(b.raw_payload, '$.place_ld.name'))`
* `url`: `COALESCE(json_extract(b.raw_payload, '$.url'), ...)`
* `city`: `COALESCE(json_extract(b.raw_payload, '$.location.city'), ...)`
* `district`: `COALESCE(json_extract(b.raw_payload, '$.location.district'), ...)`
* `price_pln`: `CAST(COALESCE(json_extract(b.raw_payload, '$.price_pln'), ...) AS REAL)`
* `area_m2`: `CAST(COALESCE(json_extract(b.raw_payload, '$.area_m2'), ...) AS REAL)`
* `rooms`: `CAST(COALESCE(json_extract(b.raw_payload, '$.rooms'), ...) AS INTEGER)`
* `floor`: `CAST(COALESCE(json_extract(b.raw_payload, '$.floor'), ...) AS INTEGER)`
* `total_floors`: `CAST(json_extract(b.raw_payload, '$.total_floors') AS INTEGER)`
* `has_elevator`: `COALESCE(CAST(json_extract(b.raw_payload, '$.has_elevator') AS INTEGER), ...)`
* `lat` / `lon`: `json_extract(b.raw_payload, '$.location.coordinates.latitude')` oraz `longitude`
* `seller_type`: `COALESCE(json_extract(b.raw_payload, '$.seller_type'), ...)`

> [!TIP]
> Zapewnienie zgodności nazw kluczy w `NieruchomosciOnlineProvider` z przyjętym schematem (`price_pln`, `area_m2`, `rooms`, `floor`, `has_elevator`, `location.coordinates`) sprawia, że widok `silver_listings` przetwarza rekordy Nieruchomosci-online w sposób całkowicie przezroczysty, bez konieczności tworzenia łamiących zmian w strukturze SQL.
...
```
Dzięki temu:
1. Oferta wystawiona jednocześnie na **Otodom**, **Adresowo** i **Nieruchomosci-online** zostaje skonsolidowana do jednego wiersza.
...
### Mechanizm Działania Audytu:
1. Podczas pierwszego zapytania do listy ofert dla danej dzielnicy, provider wyodrębnia liczbę ogłoszeń podawaną przez portal w nagłówku wyników (np. *"Znaleziono 48 ogłoszeń"* -> `expected_total = 48`).
2. Provider zlicza faktycznie sparsowane i zapisane do Bronze rekordy (`saved_bronze`).
3. Po zakończeniu pętli pobierania wywoływana jest metoda:
   ```python
   self.db_manager.save_run_audit(run_id, "nieruchomosci_online", expected_total, saved_count)
   ```
...
W procesie projektowania przeanalizowano 3 warianty implementacji providera:

```mermaid
graph TD
    subgraph Opcja1 ["Wariant 1: Dwufazowy (List + Details)"]
        A1[Pobranie Listy URL] --> B1[Pobranie Karty Każdej Oferty]
        B1 --> C1[Kompletny JSON-LD + Piętro + Rok + Winda]
    end

    subgraph Opcja2 ["Wariant 2: Jednofazowy (List Tiles Only)"]
        A2[Pobranie Listy URL] --> B2[Parsowanie Kafelków Listy]
        B2 --> C2[Brak części metadanych: winda, rok, geo]
    end

    subgraph Opcja3 ["Wariant 3: Kolejkowanie Asynchroniczne (Async/Pool)"]
        A3[Pobranie Listy URL] --> B3[Pula Workerów Asyncio/ThreadPool]
        B3 --> C3[Duża szybkość, wysokie ryzyko blokady IP/429]
    end
```
...
| **Odporność Antybotowa** | 🟢 **Wysoka**<br>Naturalne sekwencyjne odpytywanie z opóźnieniem `sleep(0.2-0.5s)` minimalizuje ryzyko 429. | 🟢 **Bardzo Wysoka**<br>Tylko kilka zapytań o strony paginacji. | 🔴 **Niska**<br>Równoczesne wysłanie dziesiątek żądań prowadzi do natychmiastowego HTTP 429 / Captcha. |
...
| **Czas Wykonania (dla 50 ofert)** | 🟡 ~15-25 sekund | 🟢 ~1-2 sekundy | 🟢 ~3-5 sekund |
...
4. **Weryfikacja obecności windy (`has_elevator`)**:
   ...
   - Źródło 2: Wzmianka w treści opisu / cechach ogłoszenia (regex `(?i)(winda|windą|windy|dźwig osobowy)`).
...
## 8. Plan Wdrożenia i Weryfikacji (Implementation Plan)

Kolejne kroki realizacyjne po akceptacji projektu technicznego:
...
2. **Weryfikacja i dostosowanie widoków SQL** w [`src/db.py`](file:///Users/pawel/git/gen-ai-orchestrator/wyszukiwarka-nieruchomosci/src/db.py) (test zgodności `silver_listings` i `gold_listings`).
3. **Podpięcie providera w `main.py`** wraz z audytem kompletności `run_audit`.
4. **Przygotowanie zestawu testów jednostkowych** w `tests/test_nieruchomosci_online_criteria.py` pokrywających:
   - Budowanie zapytań URL (filtry cenowe, pokojowe, rynkowe, dzielnice).
   - Parsowanie danych technicznych i JSON-LD.
   - Filtrowanie w warstwie Gold (winda, parter, piętra).
   - Wykrywanie duplikatów międzyserwisowych.
